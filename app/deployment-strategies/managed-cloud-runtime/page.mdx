...
```mermaid
flowchart LR
  channels["Customer Slack / Telegram"] --> runtime["Customer-sandboxed Hermes runtime"]

  subgraph cloud["Vendor-Managed Cloud"]
    runtime --> retrieval["API-backed retrieval / customer-specific knowledge store"]
    runtime --> gateway["Cloud model gateway"]
    runtime --> secrets["Customer-specific secrets"]
    runtime --> obs["Customer-specific logs and metrics"]
  end
