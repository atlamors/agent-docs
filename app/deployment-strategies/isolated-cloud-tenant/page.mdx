...
Cross-strategy assumptions are in the [Technical Appendix](../technical-appendix).

## Deployment Topology

```mermaid
flowchart LR
  channels["Customer Slack / Telegram"] --> adapters["Channel adapters"]

  subgraph tenant["Isolated Customer Cloud Tenant"]
    adapters --> hermes["Hermes runtime"]
    hermes --> queue["Task queue"]
    hermes --> rag["Customer-specific knowledge store"]
    hermes --> apiRetrieval["API-backed retrieval"]
    hermes --> workers["Connector workers"]
    hermes --> secrets["Customer secrets"]
    hermes --> audit["Logs and audit"]
    hermes --> gateway["Model gateway policy"]
  end

  apiRetrieval --> systems["Customer APIs"]
  workers --> systems["Notion / CRM / KB / project tools"]
